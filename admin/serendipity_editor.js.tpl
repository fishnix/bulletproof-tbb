...
(function(serendipity, $, undefined ) {
    ...
    serendipity.wrapSelectionWithLink = function(txtarea) {
        ...
        rel_prop = "";
        rel_prop = ' rel="nofollow"';

        if (my_link != null) {
            lft = "<a href=\"" + my_link + "\"" + html_title + rel_prop + ">";
    ...
    /* fishnix script additions */

    serendipity.popUrlModal = function(txtarea) {
        var description = serendipity.getSelection($(txtarea));
        $('#insUrlModal').modal();
        $(".modal-body #urlText").val('');
        $(".modal-body #descriptionText").val( description );
        $(".modal-body #titleText").val('');
        $(".modal-body #targetBlank").prop('checked', true);
        $(".modal-body #relNoFollow").prop('checked', true);
        $(".modal-body #textAreaId").val( txtarea[0].id );
    }

    serendipity.wrapSelectionWithLinkFromModal = function(target) {
        var urlText = $('#urlText').val();
        var descriptionText = $('#descriptionText').val();
        var titleText = $('#titleText').val();
        var targetBlank = $('#targetBlank').prop('checked');
        var relNoFollow = $('#relNoFollow').prop('checked');
        var textAreaId = $('#textAreaId').val();
        var txtArea = document.getElementById(textAreaId);

        html_title = "";
        if (titleText != "" && titleText != null) {
            html_title = ' title="' + titleText + '"';
        }
        ...
        if (urlText != null) {
            lft = "<a href=\"" + urlText + "\"" + html_title + rel_prop + target_blank + ">";

            if (descriptionText != null && descriptionText != "") {
                if (descriptionText ===  serendipity.getSelection($(txtArea))) {
                    rgt = "</a>";
                } else {
                    rgt = descriptionText + "</a>";
                }
            } else {
                rgt = "</a>";
            }

            console.log("calling wrap for ", lft + rgt);
            serendipity.wrapSelection(txtArea, lft, rgt);
        }

        return;
    }
    /* end fishnix script additions */
...
$(function() {
    ...
    /* fishnix script additions */
    $('.pop_insurlModal').click(function() {
        var target =  $('#'+serendipity.escapeBrackets($(this).attr('data-tarea')));
        serendipity.popUrlModal(target);
    });

    $('.wrap_insurlModal').click(function() {
        serendipity.wrapSelectionWithLinkFromModal();
        $('#insUrlModal').modal('hide');
    });

    $('#insUrlModal').keydown(function(event){
        if(event.keyCode==13){
            $('.wrap_insurlModal').trigger('click');
        }
    });

    $('#insUrlModal').on('shown.bs.modal', function() {
        $('#urlText').focus();
    })
    /* end fishnix script additions */
