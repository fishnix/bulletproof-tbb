{if $is_embedded != true}
    ...
    <head>
        ...
        {if $is_single_entry and not $is_preview}
                {if !empty($template_option.addthisfbimage) }
                        <link rel="image_src" href="{$template_option.addthisfbimage}" />
                {/if}
        {/if}
    ...
    <body{if $template_option.jscolumns == 'true'} onload="P7_equalCols2(0,{if $template_option.layouttype != '1col'}'content','DIV',{/if}'serendipityLeftSideBar','DIV','serendipityRightSideBar','DIV')"{/if}>
		...
		{if $template_option.facebook_comments == 'true'}<div id="fb-root"></div>{/if}
{else}
    {serendipity_hookPlugin hook="frontend_header"}
{/if}
